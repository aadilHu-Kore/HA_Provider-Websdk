    ...
    return function koreBotChat() {
        ...
        function updateOnlineStatus() {
            ...
            chatWindow.prototype.bindEvents = function () {
                ...
                //--------aadil added '.schedule'-------
                _chatContainer.off('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn,.viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply,.schedule').on('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn, .viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply,.schedule', function (e) {
                    console.log("INSIDE click GO schedule")
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    //---------------aadil added---------------------
                    if(e.currentTarget.classList && e.currentTarget.classList.length>0 && e.currentTarget.classList[0] === 'schedule') {
                        console.log("INSIDE scheduleGO")
                        var meetingtime = $(".meetingTimeValue").html();
                        var meetingDay = $(".meetingDayValue").html();
                        var startVar =  new Date(meetingDay+" "+meetingtime);
                        var timezoneVal = $(".meetingTimeZone").html();
                        var startVarLocal = moment(startVar).tz(timezoneVal).format('L, LT')
                        var startTime = new Date(startVarLocal).getTime();
                        var endTime = moment(startTime).add(30,'minutes').valueOf();
                        var meetingSchedule = {
                            "profileName":$(".profileName").html(),
                            "meetingTime":$(".meetingTimeValue").html(),
                            "meetingDay":$(".meetingDayValue").html(),
                            "timezone":$(".meetingTimeZone").html(),
                            "timeZoneValue":$(".meetingZoneValue").html(),
                            "startTime":startTime,
                            "endTime":endTime
                        }
                        $(".schedule").addClass('disabledMeeting');
                        $(".showLeftArow").addClass('disabledMeeting');
                        console.log(meetingSchedule);
                        me.sendMessage(meetingSchedule,'unique');
                    }
                    //-----------------------------------------------
                    setTimeout(function () {
                ...
                //---------------------------aadil added--------------
                var _upperBoundSlots = 12;
                _chatContainer.off('click', '.nextLinkSlots').on('click', '.nextLinkSlots', function (event) {
                    _lowerBoundSlots = _upperBoundSlots;
                    if(_upperBoundSlots + 12 >= $(".availableSlotsMainDiv").length){
                        $('.nextLinkSlots').addClass('hide');
                    }
                    _upperBoundSlots = _upperBoundSlots + 12;
                    $(".availableSlotsMainDiv").each(function(index,itemDiv){
                        if(index >= _lowerBoundSlots && index < _upperBoundSlots){
                            $(itemDiv).removeClass('hide');
                        }else{
                            $(itemDiv).addClass('hide');
                        }
                    });
                    $(".prevLinkSlots").removeClass('hide');
                    $('.chat-container').scrollTop($('.chat-container').prop('scrollHeight'));
                });
        
                _chatContainer.off('click', '.prevLinkSlots').on('click', '.prevLinkSlots', function (event) {
                    _lowerBoundSlots = _lowerBoundSlots-12;
                    _upperBoundSlots = _upperBoundSlots-12;
                    $('.nextLinkSlots').removeClass('hide');
                    if(_lowerBoundSlots <=0){
                        $(".prevLinkSlots").addClass('hide');
                    }
                    $(".availableSlotsMainDiv").each(function(index,itemDiv){
                        if(index >= _lowerBoundSlots && index < _upperBoundSlots){
                            $(itemDiv).removeClass('hide');
                ...
                _chatContainer.off('click', '.dropdown-item').on('click', '.dropdown-item', function (event) {
                    $('.timeZoneDropDown').html($(event.currentTarget).attr('data-timeZone'));
                });
        
                _chatContainer.off('keyup', '.searchTimeZone').on('keyup', '.searchTimeZone', function (event) {
                ...
                _chatContainer.off('click', '.availableSlotsMainDiv').on('click', '.availableSlotsMainDiv', function (event) {
                    $(".scheduleMeeting").addClass('hide');
                    $(".selectDay").addClass('hide');
                    $(".calender").addClass('hide');
                    $(".timeSlotsDiv").addClass('hide');
                    $(".scheduleDiv").removeClass('hide');
                    $(".meetingTimeValue").html($(this).attr('data-value'));
                    $(".meetingDayValue").html(moment(parseInt($(this).attr('data-day'))).format('MMMM D, YYYY'));
                    $(".meetingZoneValue").html(moment(parseInt($(this).attr('data-day'))).tz($('.timeZoneDropDown').html().trim()).format('z'));
                    $(".meetingTimeZone").html($(".timeZoneDropDown").html().trim());
                });
        
                _chatContainer.off('click', '.showLeftArow').on('click', '.showLeftArow', function (event) {
                    $(".scheduleMeeting").removeClass('hide');
                    $(".selectDay").removeClass('hide');
                    $(".calender").removeClass('hide');
                    $(".timeSlotsDiv").removeClass('hide');
                    $(".scheduleDiv").addClass('hide');
                });
        
                        
                $('body').on("click","#dropDown .dropdown-item",function(){
                    if($(".ui-state-default.ui-state-highlight.ui-state-active").length){
                        $(".ui-state-default.ui-state-highlight.ui-state-active").trigger('click');
                        $("#myInput").val('');
                    }else{
                        $(".ui-state-default.ui-state-active").trigger('click');
                        $("#myInput").val('');
                    }
                });
                ...
                me.bot.on("message", function (message) {
                    ...
                    if (tempData.from === "bot" && tempData.type === "bot_response") {
                        ...
                        if (me.loadHistory && me.historyLoading) {
                            ...
                            if(tempData.from === "bot" && tempData.type === "bot_response" && tempData.message[0].component && tempData.message[0].component.payload && tempData.message[0].component.payload.template_type === "date-timeslots" ){
                                console.log("INSIDE date-timeslot")
                                var enableDays = [];
                                tempData.message[0].component.payload.timeSlots = moment.tz.names();
                                tempData.message[0].component.payload.available_dates.forEach(function(dates,index){
                                    enableDays.push(moment(dates.date).format('D-M-YYYY'));
                                });
                            }
...
//----------------aadil added----------------------------------------------------
                        var availableSlotsFromPayload = [];
                        if(tempData.from === "bot" && tempData.type === "bot_response" && tempData.message[0].component && tempData.message[0].component.payload && tempData.message[0].component.payload.template_type === "date-timeslots" ){
                            var showMinDate = enableDays[0];
                            var showMaxDate = enableDays[enableDays.length -1 ];
                            availableSlotsFromPayload = tempData.message[0].component.payload.available_dates;
                            setTimeout(function(){
                                if($(".ui-state-default.ui-state-highlight.ui-state-active").length){
                                    $(".ui-state-default.ui-state-highlight.ui-state-active").trigger('click');
                                    $('.chat-container').scrollTop($('.chat-container').prop('scrollHeight'));
                            ...
                            if($('.calender#datepicker').length >= 2){
                                $('.calender#datepicker')[0].remove();
                                $('.timeSlots#dynamicTimeSlots')[0].remove();
                                $('#datepicker').datepicker({dateFormat: 'dd-mm-yy', beforeShowDay: enableAllTheseDays,minDate:showMinDate,maxDate:showMaxDate,
                                onSelect: function (date) {
                                    scheduleDate = $(this).val();
                                    var selectedDate = availableSlotsFromPayload.filter(function(timeSlotFromPayload){
                                        var dateSlotFormPayload = moment(timeSlotFromPayload.date).format('DD-MM-YYYY');
                                        if(dateSlotFormPayload === date){
                                            return true;
                                        }else{
                                            return false
                                        }
                                    });
                                    var $container = $('#dynamicTimeSlots').html('');
                                    $container = $('#dynamicTimeSlots');
                                    var dataContainer;
                                    var dataContainer1;
                                    var dataContainer2;
                                    dataContainer2 = "<div class = 'selectTime'><span>"+ 'Select Time' +"</span></div>";
                                    $container.append(dataContainer2);
                                    selectedDate.forEach(function(dateData,$index){
                                        dateData.slots.forEach(function(slotsData,$index){
                                            var dataValue = moment(slotsData).format('h:mm a');
                                            //moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a');
                                            if($index < 12){
                                                //console.log(dateData);
                                                dataContainer = "<span data-value='"+ dataValue +"' data-day = '"+ dateData.date +"' class = 'availableSlotsMainDiv'>" + moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a') + "</span>";
                                                $container.append(dataContainer);
                                            }else{
                                                dataContainer = "<span data-value='"+ dataValue +"' data-day = '"+ dateData.date +"' class = 'availableSlotsMainDiv hide'>" + moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a') + "</span>";                                        
                                                //dataContainer = "<span data-value="+ dataValue +" class = 'availableSlotsMainDiv hide'>" + moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a') + "</span>";
                                                $container.append(dataContainer);                                    
                                            }
                                        })
                                        if(dateData.slots.length > 12){
                                            dataContainer1 = "<div class = 'nextPrevDivSlots'><span class = 'prevLinkSlots hide'>"+ 'Prev' +"</span><span class = 'nextLinkSlots'>"+ 'Next' +"</span></div>";
                                            $container.append(dataContainer1);
                                        }
                                    })
                                }
                            });
                                ...
                                onSelect: function (date) {
                                    ...
                                    selectedDate.forEach(function(dateData,$index){
                                        dateData.slots.forEach(function(slotsData,$index){
                                            var dataValue = moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a');
                                            if($index < 12){
                                                //console.log(dateData);
                                                dataContainer = "<span data-value='"+ dataValue +"' data-day = '"+ dateData.date +"' class = 'availableSlotsMainDiv'>" + moment(slotsData).tz($(".timeZoneDropDown").html().trim()).format('h:mm a') + "</span>";
                                                $container.append(dataContainer);
            ...
            //----------aadil added---------------
            var scheduleDate;
            chatWindow.prototype.sendMessageOnGo = function(text){
                var meetingtime = $(".meetingTimeValue").html();
                var meetingDay = $(".meetingDayValue").html();
                var startVar = new Date(meetingDay+" "+meetingtime);
                var timezoneVal = $(".meetingTimeZone").html();
                ...
                $(".showLeftArow").addClass('disabledMeeting');
                console.log(meetingSchedule);
                this.sendMessage(meetingSchedule,'unique');
                msgData = {
                    'type': "currentUser",
                    "message": [{
                    'type': 'text',
            ...
            chatWindow.prototype.sendMessage = function (chatInput, renderMsg,msgObject,isMessageTobeHidden) {
                var me = this;
                me.stopSpeaking();
                if(renderMsg !== 'unique'){ //aadil added
            ...
            //--------------aadil added-------------------------
            if(chatInput.text().toLowerCase().trim() === 'go' || chatInput.text().toLowerCase().trim() === 'lets do it' || chatInput.text().toLowerCase().trim() === 'go ahead' || chatInput.text().toLowerCase().trim() === 'schedule'){
                //$(".schedule").trigger('click');
                this.sendMessageOnGo(chatInput.text());
                //chatInput.text('');
                return false;
            }
            //--------------------------------------------------
...
//-------------aadil added----------
                    ...
                    console.log(JSON.stringify(chatInput));
                       var clientMessageId = new Date().getTime();
                       var messageToBot = {
                           "clientMessageId":clientMessageId,
                           "message":{
